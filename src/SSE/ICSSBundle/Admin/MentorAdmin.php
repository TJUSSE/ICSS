...
class MentorAdmin extends Admin
{
    ...
    {
        $formMapper
            ->with('基本信息', array('description' => '您可以只改写学生学号、学历和专业方向，学生个人信息将在学生首次登录系统时，自动从学工网上获取。'))
    ...
    public function createQuery($context = 'list')
    {
        ...
        $query->andWhere(
            $query->expr()->eq($query->getRootAliases()[0] . '.mentor_teacher_id', ':teacher_id')
        );
        $query->setParameter('teacher_id', $this->securityContext->getToken()->getUser()->getId());
        var_dump($query);
        die();
        return;
        return $query;
